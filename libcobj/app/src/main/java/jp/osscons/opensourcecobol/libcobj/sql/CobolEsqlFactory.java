...
/**
 * 環境変数 {@code OCDB_DB_TYPE} を見て {@link CobolEsqlBackend} 実装を 1 つ生成する Factory。
 *
 * <p>現状サポートするのは PostgreSQL のみで、未指定（null/空）の場合も後方互換のため PostgreSQL を
 * 返す。新しい DB を追加する際は、対応する {@link CobolEsqlBackend} 実装を {@link #resolve(String)} の
 * 分岐に追加するだけでよい（コア側の他クラスは変更不要）。
 */
final class CobolEsqlFactory {

    /** ユーティリティクラスのインスタンス化を防ぐための private コンストラクタ。 */
    private CobolEsqlFactory() {}
     ...
     */
    static CobolEsqlBackend resolve() {
     ...
     * @return 解決された DB バックエンド
     */
    static CobolEsqlBackend resolve(String t) {
        ...
        try {
            ...
            return (CobolEsqlBackend) c.getDeclaredConstructor().newInstance();
     ...
     *
     * <p>規約：先頭 1 文字を大文字にして {@code CobolEsql} に連結する。
     * 例）{@code "db2"} → {@code CobolEsqlDb2} / {@code "postgresql"} → {@code CobolEsqlPostgresql}。
