...
ファイルロックとレコードロックの所有者は実行単位であり、JVMの各スレッドは
それぞれ独立した実行単位である。
したがって、同一JVM内の複数のスレッドは、別々のプロセスと同じ規則でINDEXEDファイルを共有する。
あるスレッドがロックしたレコードを同一JVM内の別スレッドがロック付きで読むことはできず
(ファイルステータス51)、`OPEN OUTPUT`でロックされたファイルを別スレッドが開くこともできない
(ファイルステータス61)。

実行単位が保持しているロックは、ファイルのクローズ、`STOP RUN`の実行、および
Javaアプリケーションがそのスレッドで`CobolRunUnit.end()`を呼び出したときに解放される。
