...
The same suites also run on Windows through `.github/workflows/windows-test-esql.yml`, one job per suite. There is no PostgreSQL container there: the job initialises its own cluster with `initdb --encoding=UTF8 --no-locale` from the PostgreSQL that ships with the runner image, and starts it on port 55432 — `esql-sqlca.src/connect-disconnect.at` expects a connection to `localhost:5432` to be refused, so that port has to stay closed. `esql-utf8` is excluded because the Windows build is Shift_JIS only.
