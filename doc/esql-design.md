...
On a JDBC `SQLException`, `SqlCA.setResultFromException` maps the exception's `SQLState` to a `SQLCODE` via `SqlCA.sqlStateToCode` and stores `e.getMessage()` into `SQLERRMC` (truncated to 70 bytes). Notable mappings: `02000` → `+100` (`OCPG_NOT_FOUND`), `08001`/`08003`/`28000`/`28P01` → `-402` (`OCPG_CONNECT`), `34000` → `-602`, `YE002` → `-212` (`OCPG_EMPTY`); any unrecognized state becomes `-9999` (`OCPG_UNKNOWN_ERROR`).

The `OCPG_*` codes and the `SQLSTATE`→`SQLCODE` mapping follow Open COBOL ESQL 4J. The mapping is the same except that `28000` / `28P01` (authentication failures) are mapped to `-402` (`OCPG_CONNECT`) here, whereas Open COBOL ESQL 4J does not map them and leaves them as `-9999`. One behavioral difference also follows from the transaction model: a FETCH issued on a cursor whose OPEN failed runs on the still-aborted transaction and returns the `25P02`-derived `-9999`, whereas Open COBOL ESQL 4J returns `-212` (`OCPG_EMPTY`) for that case.
...
- [esql-logging.md](./esql-logging.md) -- SLF4J logging configuration
