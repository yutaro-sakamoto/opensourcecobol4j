...
- **値**: 0以上の整数 または `INF`（デフォルト: 10000）
- **例**: `COB_FILE_IDX_COMMIT_INTERVAL=100000`
- **用途**: 索引編成ファイルの書き込みパフォーマンスを調整できます。

`OUTPUT` で開いた索引編成ファイルは排他ロックされ、`CLOSE` まで実行されるのは `WRITE` だけであるため、バックエンドのSQLiteトランザクションを `WRITE` のたびにコミットする必要はありません。代わりに、この値の件数の `WRITE` が成功するごとにコミットし、残りのレコードは `CLOSE` 時にコミットします。`INF`（大文字小文字不問）を指定すると途中のコミットを行わず、`CLOSE` 時に一括でコミットします。`0` は `1` と同じ扱いで、`WRITE` のたびにコミットします。0以上の整数とも `INF` とも解釈できない値を指定した場合は、標準エラー出力に警告を出力してデフォルト値を使用します。値を大きくする（または `INF` を指定する）とfsyncの回数が減り `WRITE` が高速になりますが、`CLOSE` 前にプロセスが強制終了・クラッシュした場合、書き込み済みのレコードが最大でこの件数失われます。`CLOSE` せずに `STOP RUN` に到達したプログラムは影響を受けません。開いたままのファイルは警告を出力した上で暗黙的にクローズされ、残りのレコードがコミットされます。また `COMMIT` 文でもそれまでに書き込んだレコードがコミットされます。`INPUT`・`I-O`・`EXTEND` で開いたファイルには影響しません。重複キーの検出（ファイルステータス21/22）にも影響はなく、従来通り `WRITE` のたびに行われます。
