...
- **値**: `WAL`（既定値）または `DELETE`。大文字小文字は区別しません。それ以外の空でない値を指定した場合は標準エラー出力に警告を表示し、`WAL` として扱います。
...
ジャーナルモードはSQLiteファイルのヘッダに保存されます。ランタイムはINDEXEDファイルを開くたびにこの設定を適用するため、環境変数を切り替えれば既存のファイルも両方向に変換されます。他のプロセスがそのファイルを開いている間は、エラーにはならず変換が見送られます。ファイルは現在のモードのままとなり、次に使われていない状態でOPENされたときに変換されます。`cobj-idx migrate` と `cobj-idx unlock` もこの設定を適用します。

多くの `COB_*` 変数と異なり、この変数はプロセス起動時に一度だけ読み取られ、COBOLの `SET ENVIRONMENT` 文の影響を受けません。

WALモードでINDEXEDファイルを開いている間、SQLiteは同じディレクトリに `<ファイル名>-wal` と `<ファイル名>-shm` の2つの補助ファイルを作成します（`DELETE` モードでは `<ファイル名>-journal`）。これらは最後の接続が正常にクローズされた時点で自動的に削除され、`DELETE FILE` は主ファイルと一緒にこれらも削除します。プログラムが異常終了した場合は `-wal` が残りますが、その内容は次にファイルを開いたときに復元されます。ただしその状態でINDEXEDファイルをコピーする場合は `-wal` も一緒にコピーしないと、そこに保持されているレコードが失われます。
