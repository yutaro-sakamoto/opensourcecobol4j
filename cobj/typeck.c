...
static cb_tree cb_build_move_literal(cb_tree src, cb_tree dst) {
  ...
  if (l->all) {
     ...
     * NATIONAL は CobolNationalField.moveFrom(String) が
     * moveFrom(AbstractCobolField) に委譲する正しい実装なのでサポート対象。
     * 編集系や数値系は libcobj 側の moveFrom(String) が no-op/不完全なので
     * 別途 libcobj を整備したうえで段階対応する。 */
